function [result] = ...
    fun_p_online_test_thresh(path_head,path_toolbox,path_data,subjectID,...
...
% save the result of classifier 1 and the probability of classifier2
label_classifier1 = cell(1,length(index));
probability_classifier2 = cell(1,length(index));
label_classifier2 = cell(1,length(index));
label_groundtruth = cell(1,length(index));

% 
thresh_classifier2 = 3; % temp thresh, no use
...
for num_choose = 1:length(index)
    ...
    for i = 1:8
        [b{i+2},a{i+2}]= butter(4,[50*i-0.5,50*i+0.5]/(fs_emg/2),'stop');
    end
...
%         disp(ntime);
        ...
        if ntime < window_len
            label1(id_time) = 0;
            label2(id_time) = 0;
            prob2(1:length(index),id_time) = 0;
...
%             end
        ...
        if ges_times_1 > floor(num_win/2) && ges_mode_1 == 1
            label_predict_1 = 1;
        else
            label_predict_1 = 0;
        end
        ...
        if label_predict_1 == 1 % motion:1; rest:0
            ...
            % [probability_label_predict_tmp_2,label_predict_tmp_2] = ...
            %    max(sum_labels_2_nor,[],2);
...
%             [ges_mode_2, ges_times_2] = mode(label_buffer_now(2,:));
            ...
            sum_labels_2_nor = zeros(1,length(index));
            probability_mdl2_buffer_now(num_win) = 0;
            %             label2_now = 0;
        end

        label2(id_time) = label_predict_2;
        prob2(:,id_time) = sum_labels_2_nor';
    ...
    label_groundtruth{num_choose} = label_real;
    ...
    label_classifier1{num_choose} = label1;
    probability_classifier2{num_choose} = prob2;
    label_classifier2{num_choose} = label2;
%     figure(num_choose);
%     plot(label)
%     hold on
%     plot(label_real,'r');
...
result.label_classifier1 = label_classifier1;
result.label_classifier2 = label_classifier2;
result.probability_classifier2 = probability_classifier2;
result.label_groundtruth = label_groundtruth;
