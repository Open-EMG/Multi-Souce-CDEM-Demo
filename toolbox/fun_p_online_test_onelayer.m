function [acc_rest_all,acc_gesture_all,acc] = fun_p_online_test_onelayer(path_head,path_toolbox,path_data,subjectID,...
    sessionID,subject_test_ID,session_test,index,thresh_classifier)
...
%     label2 = [];    
    ...
    load([path_data,subject_test,num2str(session_test),'/model_onelayer.mat']);
...
%     label_buffer_now = zeros(2,num_win);
    label_buffer = zeros(1,num_win);
    label_buffer_now = zeros(1,num_win);
    probability_mdl_buffer_now = zeros(length(index)+1,num_win);
    label1 = [];    
%     label2 = [];
...
%             end
        ...
        for i = 1:n_model
            if isempty(mdl1{1,i})
                label_predict_tmp_1(i) = 1; % doesn't matter because corresponding acc_target eqauls to 0 
                continue
            end
            mdl1_tmp = mdl1{1,i};
            %         mdl2_tmp = mdl2{1,i};
        ...
        sum_labels_1 = acc_target1*onehot(label_predict_tmp_1,length(index)+1);
        sum_labels_1_nor = sum_labels_1/sum(sum_labels_1);

        probability_mdl_buffer_now(:,1:num_win-1) = probability_mdl_buffer_now(:,2:num_win);
        probability_mdl_buffer_now(:,num_win) = sum_labels_1_nor';

        sum_probability_mdl_buffer_now = sum(probability_mdl_buffer_now,2);
        [probability_tmp,label_predict_tmp_1] = max(sum_probability_mdl_buffer_now);

        if label_predict_tmp_1 == 11
            label_predict_tmp_1 = 0;
        end
        label_buffer_now(:,1:num_win-1) = label_buffer(:,2:num_win);
        label_buffer_now(1,num_win) = label_predict_tmp_1;

        [ges_mode_1] = mode(label_buffer_now(1,:));

        if probability_tmp > thresh_classifier && ges_mode_1 ~= 0
            label_predict_1 = ges_mode_1;
        else
            label_predict_1 = 0;
        end
...
%             [~,label_predict_tmp_2] = max(sum_labels_2,[],2);
% 
%             label_buffer_now(2,num_win) = label_predict_tmp_2;
%             [ges_mode_2, ges_times_2] = mode(label_buffer_now(2,:));
%             if ges_times_2 > floor(num_win/2) && ges_mode_2 ~= 0
